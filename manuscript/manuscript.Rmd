...
#Methods
We evaluated associated chlorophyll *a* concentrations for an effect for each of the WHO and U.S. EPA levels (Table \ref{tab:microcystin_levels}). Lakes with higher microcystin-LR concentrations were rare. Only `r round(100*(length(nla_dat$MCYST_TL_UGL[nla_dat$MCYST_TL_UGL>=10])/nrow(nla_dat)),2)` % of lakes sampled had a concentration greater than 10.  For this analysis we focus on the microcystin concentrations that are better represented in the NLA data. These were  0.3, 1, 1.6, and 2 $\mu$g/L.
...
```{r epa_child_cp_plot,echo=FALSE, fig.cap="Plot of \\label{fig:epa_child_cp_plot}"}
plot_cp(epa_child_cp,x = "Log 10 Chlorophyll", y = "Probability of Exceeding EPA Child HA")
```

\newpage

```{r who_drink_cp_plot,echo=FALSE, fig.cap="Plot of  \\label{fig:who_drink_cp_plot}"}
plot_cp(who_drink_cp, x = "Log 10 Chlorophyll", y = "Probability of Exceeding WHO Drinking HA")
```

\newpage

```{r epa_adult_cp ,echo=FALSE, fig.cap="Plot of  \\label{fig:epa_adult_cp}"}
plot_cp(epa_adult_cp, x = "Log 10 Chlorophyll", y = "Probability of Exceeding EPA Adult HA")
```

\newpage

```{r who_rec_low1_cp, echo=FALSE, fig.cap="Plot of  \\label{fig:who_rec_low1_cp}"}
...
```{r microcystin_levels,echo=FALSE,results='asis', fig.cap="Various suggested microcystin-LR health advisory concentrations. \\label{fig:microcystin_levels}"}
...
```{r mc_chla_table, results='asis', echo=FALSE, fig.cap="Chlorophyll \textit{a} concentrations that are associated with a 50% probability of exceeding a microcystin-LR health advisory concentration. \\label{fig:mc_chla_table}"}
