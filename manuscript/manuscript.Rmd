...
mc_chla<-cbind(as.character(epa_child),as.character(who_drink),
               as.character(epa_adult),as.character(who_rec))
...
total_accuracy<-NULL
total_accuracy$epa_child<-https://github.com/USEPA/Microcystinchla/blob/master/R/get_nla.R((child_conmat[1,1]+child_conmat[2,2])/
  sum(child_conmat),2)
...
plot_scatter(nla_dat,xvar="log_chla",yvar = "log_micro",pt_col = nla_dat$WSA_ECO9,
...
```{r epa_child_cp_plot, echo=FALSE, fig.width=6, fig.cap="Conditional probability plots showing association between the probability of exceeding various microcystin health advisory Levels. A.) Plot for USEPA Child (0.3 $\\mu$g/L). B.) Plot for WHO Drinking (1 $\\mu$g/L). C.) Plot for USEPA Adult (1.6 $\\mu$g/L). D.) Plot for WHO Recreational (2 $\\mu$g/L). \\label{fig:multi_cp_plot}"}
multiplot(
...
```{r microcystin_levels,echo=FALSE,results='asis', tab.cap="Various suggested microcystin health advisory concentrations. \\label{tab:microcystin_levels}"}
